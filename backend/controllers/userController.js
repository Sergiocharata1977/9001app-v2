...
const createOrganizationUser = async (req, res) => {
  try {
    ...
    const newUserResult = await mongodbClient.execute({
...
const updateOrganizationUser = async (req, res) => {
  try {
    ...
    const userResult = await mongodbClient.execute({
    ...
    if (email) {
      ...
      const emailCheck = await mongodbClient.execute({
    ...
    const updatedUserResult = await mongodbClient.execute({
...
const createOrganization = async (req, res) => {
  try {
    ...
    const existingAdmin = await mongodbClient.execute({
      sql: 'SELECT id FROM usuarios WHERE email = ?',
      args: [adminEmail]
    });
    ...
    const adminResult = await mongodbClient.execute({
      sql: `INSERT INTO usuarios (name, email, password_hash, role, organization_id, created_at) 
            VALUES (?, ?, ?, ?, ?, ?)`,
