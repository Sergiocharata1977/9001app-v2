...
class CoordinacionService {
  ...
  async obtenerTareas(organizationId = 2, limit = 50, offset = 0) {
    try {
      ...
      const result = await mongoClient.execute(query, [organizationId, limit, offset]);
      return result.rows;
    } catch (error) {
  ...
  async obtenerTareaPorNumero(organizationId, tareaNumero) {
    try {
      ...
      const result = await mongoClient.execute(query, [organizationId, tareaNumero]);
  ...
  async obtenerEstadisticas(organizationId = 2) {
    try {
      ...
      // Por prioridad
      const prioridadQuery = await mongoClient.execute(`
      ...
      // Tiempo total
      const tiempoQuery = await mongoClient.execute(`
        SELECT 
  ...
  async obtenerTareasPorModulo(organizationId, modulo, limit = 20) {
    try {
      ...
      const result = await mongoClient.execute(query, [organizationId, modulo, limit]);
      return result.rows;
    } catch (error) {
  ...
  async obtenerTareasPorEstado(organizationId, estado, limit = 20) {
    try {
      ...
      const result = await mongoClient.execute(query, [organizationId, estado, limit]);
      return result.rows;
    } catch (error) {
  ...
  async crearTarea(tareaData) {
    try {
      ...
      const result = await mongoClient.execute(query, params);
  ...
  async obtenerSiguienteNumeroTarea(organizationId) {
    try {
      ...
      const result = await mongoClient.execute(query, [organizationId]);
