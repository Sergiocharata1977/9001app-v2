...
router.get('/search', async (req, res) => {
  try {
    ...
    const countResult = await mongodbClient.execute({
      sql: `
...
router.post('/', async (req, res) => {
  try {
    ...
    const newHallazgo = await mongodbClient.execute({
      sql: `
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const existingHallazgo = await mongodbClient.execute({
    ...
    const updatedHallazgo = await mongodbClient.execute({
      sql: `
        SELECT h.*, (resp.nombres || ' ' || resp.apellidos) as responsable_nombre
        FROM hallazgos h
        LEFT JOIN personal resp ON h.responsable_id = resp.id
...
router.get('/health', async (req, res) => {
  try {
    const result = await mongodbClient.execute('SELECT COUNT(*) as count FROM hallazgos WHERE is_active = 1');
