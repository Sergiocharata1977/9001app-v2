...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const lastAccionResult = await mongodbClient.execute("SELECT numeroAccion FROM acciones ORDER BY numeroAccion DESC LIMIT 1");
    ...
    const newAccionResult = await mongodbClient.execute({
...
router.get('/', async (req, res) => {
    ...
    try {
        const result = await mongodbClient.execute({ sql: query, args });
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        const updatedAccionResult = await mongodbClient.execute({
            sql: 'SELECT * FROM acciones WHERE id = ?',
            args: [id]
        });
