...
router.get('/con-relaciones/:id', async (req, res) => {
  try {
    ...
    const personalResult = await mongodbClient.execute({
      sql: `SELECT p.*, o.name as organization_name 
            FROM personal p 
            LEFT JOIN organizations o ON p.organization_id = o.id 
    ...
    const puestosResult = await mongodbClient.execute({
    ...
    const departamentosResult = await mongodbClient.execute({
...
router.post('/:id/asignar-puesto', async (req, res) => {
  try {
    ...
    const personalExists = await mongodbClient.execute({
    ...
    const puestoExists = await mongodbClient.execute({
...
router.post('/:id/asignar-departamento', async (req, res) => {
  try {
    ...
    const departamentoExists = await mongodbClient.execute({
