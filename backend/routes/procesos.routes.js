...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const procesoResult = await mongodbClient.execute({
    ...
    const participantesResult = await mongodbClient.execute({
    ...
    const documentosResult = await mongodbClient.execute({
    ...
    // Obtener normas SGC
    const normasResult = await mongodbClient.execute({
...
router.post('/:id/documentos', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/:id/normas', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    const orgId = req.user?.organization_id || 2;
...
router.post('/:id/normas', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    const { 
...
router.get('/dashboard/sgc', authMiddleware, async (req, res) => {
  try {
    const orgId = req.user?.organization_id || 2;
