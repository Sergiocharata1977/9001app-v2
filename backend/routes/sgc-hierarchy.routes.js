...
router.get('/procesos/:id/hierarchy', authMiddleware, async (req, res, next) => {
  try {
    ...
    const proceso = await mongodbClient.execute({
    ...
    if (indicadorIds.length > 0) {
      const medicionesResult = await mongodbClient.execute({
